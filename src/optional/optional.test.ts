...
await test("Optional", async (t) => {
    ...
    await t.test(
        ...
        () => {
            ...
            assert.deepStrictEqual(
                objectMaybeValue.value_or({ key: "default" }),
                {
                    key: "default",
                }
            );
    ...
    await t.test(
        "Won't run the mapper function if the Optional is empty",
        () => {
            const emptyMaybeValue = optional.none<string>();
            const transformed = emptyMaybeValue.map(() =>
                assert.fail(
                    "Mapping function was run on an empty Optional value."
                )
            );
            assert.ok(!transformed.is_some());
        }
    ...
    await t.test("can chain operations using .or_else()", () => {
        ...
        const emptyMaybeValue = optional.none<string>();
        const emptyResult = emptyMaybeValue.or_else(() =>
    ...
    await t.test("Generator/Iterator Interface", async (t) => {
        await t.test(
            "Optional with value yields its value in for-of loop",
            () => {
        ...
        await t.test(
            "can collect present values from array of optionals",
            () => {
                const optionals = [
                    optional.some(1),
    ...
    await t.test("from() method", async (t) => {
        ...
        await t.test(
            "works with functions that conditionally return null",
            () => {
    ...
    await t.test("from_nullable() method", async (t) => {
        await t.test("wraps non-null/undefined values in some", () => {
            ...
            const numberResult = optional.from_nullable(
                [42].find((v) => v === 42)
            );
            assert.ok(numberResult.is_some());
            assert.strictEqual(numberResult.value, 42);
    ...
    await t.test("from_async() method", async (t) => {
        ...
        await t.test(
            ...
            async () => {
                ...
                const foundResult = await optional.from_async(() =>
                    fetchUser(1)
                );
                assert.ok(foundResult.is_some());
                assert.deepStrictEqual(foundResult.value, { name: "Alice" });

                const notFoundResult = await optional.from_async(() =>
                    fetchUser(2)
                );
                assert.ok(!notFoundResult.is_some());
            }
        ...
        await t.test("can be chained with other Optional methods", async () => {
            const result = (
                await optional.from_async(() => Promise.resolve("hello"))
            )
                .map((v) => v.toUpperCase())
                .and_then((v) => optional.some(v.length))
                .value_or(0);
        ...
        await t.test(
            ...
            async () => {
                ...
                const zeroResult = await optional.from_async(() =>
                    Promise.resolve(0)
                );
                assert.ok(zeroResult.is_some());
                assert.strictEqual(zeroResult.value, 0);

...
await test("Async Results", async (t) => {
    await t.test("Support the full Result interface", async (t) => {
        ...
        await t.test("error_or() works on async result", async () => {
            ...
            const successResult = result.try_async(() =>
    ...
    await t.test("try_async method", async (t) => {
        ...
        await t.test("try_async() chains with other methods", async () => {
            const chained = createSuccessResult("hello")
                ...
                .and_then((s) =>
                    s.length > 5
                        ? result.ok(s)
                        : result.error(new Error("Too short"))
                );
            const res = await chained;
        ...
        await t.test(
            "try_async() handles non-Error thrown values",
            async () => {
                const asyncResult = createSuccessResult("initial");
                const chained = asyncResult.try_async(() => {
                ...
                assert.strictEqual(
                    error.message,
                    "Async operation failed: String error"
                );
    ...
    await t.test("try_async with custom error mapping", async (t) => {
        ...
        await t.test(
            "try_async() error mapping preserves type union",
            async () => {
        ...
        await t.test(
            "try_async() with error mapper chains correctly",
            async () => {
                const chained = createSuccessResult("hello")
                    .try_async(
                        () => {
                        ...
                        (error) =>
                            new CustomError(`Step 1: ${String(error)}`, 400)
                    )
                    .try_async(
                        () => Promise.resolve("should not be called"),
                        (error) =>
                            new CustomError(`Step 2: ${String(error)}`, 500)
                    );
                const res = await chained;
    ...
    await t.test("Edge cases and concurrent operations", async (t) => {
        ...
        await t.test(
            ...
            async () => {
                ...
                const errorOp = createSuccessResult("initial").try_async(
                    async () => {
                        await new Promise((resolve) => setTimeout(resolve, 5));
